...
pruneset([],[]).
pruneset([(S :- U1)],[(S :- U21)]) :-
	removeConstraints(U1,U11),
	removeDupls(U11,U2),
	pruneAtoms(U2,U),
	canonical_each(U),
	addConstraints(U,U21).
	
removeConstraints([],[]).
removeConstraints([(S :- _)|Xs],[S|Ys]) :-
	removeConstraints(Xs,Ys).
	
	
addConstraints([],[]).
addConstraints([S|Ys],[(S :- true)|Xs]) :-
	addConstraints(Ys,Xs).
...
indexBody((solutions(B,Ss),Bs),[atom(N,L,solutions(B,Ss))|Bs1],N,M) :-
	!,
	N1 is N+1,
	length(Ss,L),
...
indexBody(true,[],N,N) :-
	!.
indexBody(solutions(B,Ss),[atom(N,L,solutions(B,Ss))],N,N) :-
	length(Ss,L).
...
findV([atom(N,_,solutions(B,_))|Bs],V,[N|Ns],Ws,N) :-
	vars(B,Xs),
	memb3(V,Xs),
	!,
	N1 is N+1,
...
%	output C = (solutions(p(X),[p(a),p(b)]), solutions(q(X), [q(d),q(b),q(e)]))
...
solveBody(H,Bs,Sols,BMap,K) :-
	...
	findall((H:-Cs),solveAndProject(B1,Cs),Sols1),
	simplifyConstraints(Sols1,Sols).
...
eliminateLocals(BMap,Bs,B,K) :-
	...
	setdiff(Ws,Ys,Us),
	G =.. [temp|Us],
	findall((G:-Cs),solveAndProject(ABs,Cs),Sols1), 	% eliminate locals Ys, return solutions for Us
	simplifyConstraints(Sols1,Sols2),
	K1 is K+1,
	canonical_each(Sols2),
	removeDupls(Sols2,Sols3),
	pruneAtoms(Sols3,Sols4),
	length(Sols4,L),
	adjustMap(BMap1,As,Us,Ys,K1,BMap2),
	eliminateLocals(BMap2,[atom(K1,L,solutions(G,Sols4))|Bs1],B,K1).
	
solveAndProject(true,true).
solveAndProject((solutions(B,As),Bs),(C,Cs)) :-
	member((B:-C),As),
	solveAndProject(Bs,Cs).
solveAndProject(solutions(B,As),C) :-
	member((B:-C),As).
	
simplifyConstraints([],[]).
simplifyConstraints([(H :- Cs)|Sols],[(H :- Cs1)|Sols1]) :-
	vars(H,Vs),
	mergeConstraints(Vs,Cs,Cs1),
	!,
	simplifyConstraints(Sols,Sols1).
simplifyConstraints([_|Sols],Sols1) :-
	simplifyConstraints(Sols,Sols1).
	
mergeConstraints([],Cs,true) :-
	satisfiable(Cs),
	!.
mergeConstraints([V],Cs,C) :-
	selectV(Cs,V,Ds,Cs1),
	newConstraint(Ds,V,C),
	satisfiable(Cs1),
	!.
mergeConstraints([V,V1|Vs],Cs,(C,Cs1)) :-
	selectV(Cs,V,Ds,Cs1),
	newConstraint(Ds,V,C),
	mergeConstraints([V1|Vs],Cs,Cs1).
	
newConstraint([],_,true).
newConstraint([D|Ds],V,member(V,S)) :-
	intersectAll([D|Ds],S).
...
selectV(true,_,[],true).
selectV((member(V,D1),Cs),V1,[D1|Ds],Cs1) :-
	V==V1,
	!,
	selectV(Cs,V1,Ds,Cs1).
selectV((C,Cs),V1,Ds,(C,Cs1)) :-
	!,
	selectV(Cs,V1,Ds,Cs1).
selectV(member(V,D1),V1,[D1],true) :-
	V==V1,
	!.
selectV(C,_,[],C).
	
satisfiable(true).
satisfiable((member(X,D),Cs)) :-
	member(X,D),
	satisfiable(Cs).
satisfiable((true,Cs)) :-
	satisfiable(Cs).
satisfiable(member(X,D)) :-
	member(X,D).
...
compareSolns((H1 :- S1),(H2 :- S2),I,D1,D2) :-
	H1 =.. [P|Xs],
	H2 =.. [P|Ys],
	compareArgs(Xs,S1,Ys,S2,I,D1,D2).
	
compareArgs([],_,[],_,[],[],[]).
compareArgs([X|Xs],S1,[Y|Ys],S2,I,D1,D2) :-
	var(X),
	var(Y),
	!,
	compareArgs(Xs,S1,Ys,S2,I,D1,D2).
