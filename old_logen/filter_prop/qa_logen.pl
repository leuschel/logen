:- module(qa_logen, [qaLogenTrans/3]).
...
qa_read_clauses([],Ps,QA,QA,_) :- 
	!,
	close_list(Ps).
qa_read_clauses([clause((ann_clause(_,H,B):-true),Vs)|Cs],Ps,Out,Out2,I) :- 
	!,
	I1 is I+1,
	make_qa_clauses(clause((H :- B),Vs),Ps,Out,Out1,I1),
	qa_read_clauses(Cs,Ps,Out1,Out2,I1).
qa_read_clauses([clause((filter(T,Modes) :- true),_)|Cs],Ps,Out,Out2,I) :- 
	!,
	make_filter_clauses(T,Modes,Ps,Out,Out1),
	qa_read_clauses(Cs,Ps,Out1,Out2,I).
...
removelast((B,C,Bs),Bn,(B,Bs1),N) :-
	...
	N is N1+1.
removelast((B,C),C,B,2) :-
	!.
removelast(B,B,true,1).


answerlits((rescall(_),Bs),Bs1) :-
	!,
	answerlits(Bs,Bs1).
answerlits((memo(_),Bs),Bs1) :-
	!,
	answerlits(Bs,Bs1).
answerlits((call(B),Bs),(B,Bs1)) :-	% call to builtin
	builtin(B),
	!,
	answerlits(Bs,Bs1).
answerlits((unfold(B),Bs),(B,Bs1)) :-
	!,
	answerlits(Bs,Bs1).
answerlits((B,Bs),(B,Bs1)) :-
	...
	nl(user_error),
	answerlits(Bs,Bs1).
answerlits(rescall(_),true) :-
	!.
answerlits(memo(_),true) :-
	!.
answerlits(call(B),B) :-
	builtin(B),
	!.
answerlits(unfold(B),B) :-
	!.
...
make_filter_clauses(T,Modes,Ps,[PFilterCl|Out],Out) :-
	functor(T,P,N),
	remove_dynamic(Modes,Ms),
	PFilter =.. [P|Ms],
	flat_name(query(PFilter),QPF),
	functor(QPF,QP,N),
	memb1(QP/N,Ps),
	PFilterCl = clause((QPF :- true), []).
	
remove_dynamic([],[]).
remove_dynamic([dynamic|Modes],[_|Ms]) :-
	!,
	remove_dynamic(Modes,Ms).
remove_dynamic([M|Modes],[M|Ms]) :-
	remove_dynamic(Modes,Ms).