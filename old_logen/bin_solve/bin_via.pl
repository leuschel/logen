...
bin_solve(PP,[unfold(PP1,H)|_T],RecCall) :-
	via(PP1),
        old_bin_solve_atom(PP,H,RecCall).
...
solve_file(Filename) :-
	...
	filtered(H),
	solve_atom(H),
	program_point_for(H,PP), /* not required anymore */
        program_point_for(Rec,PP), /* not required anymore */
...
convert_call(logen(Ann,Call), ACall) :-
	...
	assert(program_point_for(Call,PP)).
